...
interface CalendarDateValue extends DateValue {
  isDisabled: boolean;
}
  ...
  const calendarDays = computed(() => {
    ...
    for (let i = startingDayIndex - 1; i >= 0; i--) {
      days.push({
        ...
        isDisabled: false, // Previous month dates are not disabled
      });
    }
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push({
        ...
        dayOfMonth: i,
        isCurrentMonth: true,
        isDisabled: props.disabledDates.some(
    ...
    for (let i = 1; i <= remainingDays; i++) {
      days.push({
        ...
        dayOfMonth: i,
        isCurrentMonth: false,
        isDisabled: false, // Next month dates are not disabled
      });
    }
